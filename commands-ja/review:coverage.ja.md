...
- コンポーネント間の相互作用をテスト
- 実用的な場合は実際の依存関係を使用
- データベース、ファイルシステム、または外部サービスを含む場合がある
- データフローと統合コントラクトを検証
- コンポーネント間のエラー伝播をテスト
- テストデータベースまたはコンテナを使用
- 実行後にクリーンアップすべき
...
- 遅い実行時間を受け入れる
- 安定で保守可能であるべき
- 数を最小限に抑える(テストピラミッドの頂点)
- 目標: クリティカルなユーザーフローをカバー

## カバレッジツールリファレンス

カバレッジを分析する際、一般的なツールを認識する:

### JavaScript/TypeScript
- **Jest**: Istanbulを使用した組み込みカバレッジ
- **NYC**: Istanbul コマンドラインインターフェース
- **Codecov/Coveralls**: カバレッジレポートサービス

### Python
- **Coverage.py**: 標準的なPythonカバレッジツール
- **pytest-cov**: カバレッジのためのpytest統合

### Java/JVM
- **JaCoCo**: Javaコードカバレッジライブラリ
- **Cobertura**: Javaプロジェクト用カバレッジツール

### Go
- **go test -cover**: 組み込みカバレッジツール
- **gocov**: 代替カバレッジツール

### Ruby
- **SimpleCov**: Ruby用コードカバレッジ

### C#/.NET
- **Coverlet**: クロスプラットフォームコードカバレッジ
- **dotCover**: JetBrainsカバレッジツール
...
## 技術固有の考慮事項
以下に固有のテスト特性を考慮:
- **Webアプリケーション**: UIコンポーネントテスト、APIテスト、ブラウザ自動化
- **API**: コントラクトテスト、リクエスト/レスポンス検証、エラー処理
- **データベース**: データアクセス層テスト、クエリテスト、マイグレーションテスト
- **マイクロサービス**: サービス統合テスト、APIコントラクトテスト、レジリエンステスト
- **フロントエンドフレームワーク**: コンポーネントテスト、状態管理テスト、ユーザーインタラクションテスト
- **バックエンドフレームワーク**: コントローラテスト、サービス層テスト、リポジトリテスト
- **モバイルアプリケーション**: プラットフォーム固有テスト、UIテスト、オフライン動作

## ミューテーションテストの考慮
このレビューはカバレッジメトリクスとテスト品質に焦点を当てていますが、ミューテーションテストの推奨を検討:
- ミューテーションテストはテストが実際にバグを捕まえるかを検証
- コードを変更(ミューテート)してテストが失敗するかをチェック
- 良好なカバレッジを持つ非効果的なテストを特定するのに役立つ
- ツール: Stryker (JS)、PITest (Java)、mutmut (Python)
- テスト有効性のカバレッジ分析を補完
