...
**スキルファミリー（例外）:** 単一スキルを分割せず、密結合した複数スキルとして提供したい場合（例: 収集役とオーケストレーション役を分ける）は、共通の親ディレクトリの下にネストしてよい: `skills/<family-name>/<sub-skill-name>/SKILL.md`（例: `skills/pr-knowledge/collect/SKILL.md`、`name: collect`）。この場合の呼び出し名は `<family-name>:<sub-skill-name>`（例: `pr-knowledge:collect`）になる。単に関連があるだけの独立したスキル同士は、この節冒頭の原則どおりフラットな命名（例: `review-after`）でグループを表現し、ネストは使わない。
...
**例外:** 上記は「たまたま関連がある」独立したスキル同士の整理法。これに対し、1つの機能を役割ごとに分割した密結合なスキル群（例: 知見を集める役と、それらを束ねるオーケストレーション役）は、[ファイル命名と配置](#ファイル命名と配置)の「スキルファミリー」節に従いネスト構成を使う：
